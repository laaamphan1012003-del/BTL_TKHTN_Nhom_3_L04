...
class App:
    ...
    def login(self):
        ...
        output = output.decode('utf-8')   # chuyển bytes thành string
        name = output.split(',')[1].strip()
        print(name)
        
        if name in ['no_persons_found','unknown_person']:
            util.msg_box("Login Failed", "No matching user found. Please try again or regist a new user.")
        else:
            util.msg_box("Login Success", "Welcome, {}!".format(name))
    ...
    def add_img_to_label(self, label):
         imgtk = ImageTk.PhotoImage(image=self.most_recent_capture_pil) # convert image for tkinter
         label.imgtk = imgtk
